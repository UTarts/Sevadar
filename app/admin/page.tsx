...
export default function AdminDashboard() {
  ...
  const handleFeedSubmit = async () => {
      ...
      if (!error) { alert("Posted!"); setFeedTitle(''); setFeedDesc(''); setFeedFiles([]); setFeedUrl(''); loadAllData(); }
      setUploading(false);
  };
  ...
  // --- SKELETON COMPONENT ---
  const SkeletonRow = () => (
    <div className="p-4 border-b flex items-center gap-4 animate-pulse">
  ...
  const SkeletonCard = () => (
    <div className="bg-white p-4 rounded-xl shadow-sm border animate-pulse space-y-3">
    ...
    <div className="min-h-screen bg-gray-50 font-sans pb-24">
          ...
          <div className="flex justify-between items-center mb-6">
             ...
             <div className="flex items-center gap-2">
                 <button 
                    ...
                    className="p-2 bg-gray-100 rounded-full text-gray-600 hover:bg-orange-50 hover:text-orange-600 transition-colors font-bold text-xs flex items-center gap-1"
                 >
                    <Languages size={18}/> {language === 'hi' ? 'हिंदी' : 'EN'}
                 </button>
      ...
      <div className="p-4">
          ...
          {activeTab === 'suggestions' && (
              <>
                <div className="flex justify-between mb-4">
                    <h2 className="text-lg font-bold">Inbox: {loading ? '...' : messages.length}</h2>
          ...
          {activeTab === 'feed' && (
              <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                      {loading ? [1,2].map(i => <SkeletonCard key={i}/>) : existingPosts.map(p => (
                          <div key={p.id} className="bg-white p-3 rounded-xl shadow-sm border flex gap-3">
                              <div className="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden shrink-0">{p.type==='image'?<img src={p.images?.[0]||p.url} className="w-full h-full object-cover"/>:<Video className="m-auto mt-4 text-gray-400"/>}</div>
                              <div className="flex-1"><h4 className="font-bold text-sm line-clamp-1">{p.title}</h4><button onClick={()=>deleteFeed(p.id)} className="text-red-500 text-[10px] font-bold mt-1 flex items-center gap-1"><Trash2 size={12}/> Delete</button></div>
                          </div>
                      ))}
                  </div>
              </div>
          ...
          {activeTab === 'polls' && (
              <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                      {loading ? [1,2].map(i => <SkeletonCard key={i}/>) : activePolls.map(p => (
                          <div key={p.id} className="bg-white p-4 rounded-xl shadow-sm border flex justify-between items-start"><div><h4 className="font-bold text-sm">{p.question}</h4></div><button onClick={()=>deletePoll(p.id)} className="text-red-500"><Trash2 size={16}/></button></div>
                      ))}
                  </div>
              </div>
          )}
          ...
          {activeTab === 'quiz' && (
              <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                      ...
                      {loading ? [1,2].map(i => <SkeletonCard key={i}/>) : quizzes.map(q => (
